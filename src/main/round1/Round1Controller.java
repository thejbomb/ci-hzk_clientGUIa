...
public class Round1Controller extends MainController implements Initializable, Runnable, ServerInteractionInterface, ServerHandlerInterface {
    ...
    protected void show(){
        gp_round1.setVisible(true);
    }

    private void hide(){
        gp_round1.setVisible(false);
    }
    ...
    public void handleServerData(int command, LinkedList<String> data) {
        switch (command) {
            case Constants.BEGIN_R1L1:
                ...
                break;
            case Constants.S2C_R1L1_SCR:
                userData.setRound1Point(Integer.parseInt(data.getFirst()));
                lb_userPoint1.setText(data.getFirst());
                System.out.println(userData.getName() + " current total point for round 1 is " + data.getFirst());
                break;
