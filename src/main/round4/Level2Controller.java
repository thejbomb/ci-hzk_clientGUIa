...
public class Level2Controller extends Round4Controller implements Initializable, TimerInterface {
    ...
    private void setData() {
        lb_instructionBody_zh.setText(Main.R4L2_DATA.getINSTRUCTION_ZH());
        lb_instructionBody_en.setText(Main.R4L2_DATA.getINSTRUCTION_EN());
        String timeLimit_zh = "限时" + Main.R4L2_DATA.getTIME_LIMIT() + "分钟";
        String timeLimit_en = "Time Limit: " + Main.R4L2_DATA.getTIME_LIMIT() + (Main.R4L2_DATA.getTIME_LIMIT() > 1 ? " minutes" : " minute");
        lb_instructionTime_zh.setText(timeLimit_zh);
        lb_instructionTime_en.setText(timeLimit_en);

        lb_exampleBody.setText(Main.R4L2_DATA.getEXAMPLES().get(0));
    }
    ...
    private void displayChoices(int questionNumber) throws Exception {
        ...
        for (int i = 0; i < choices.length; i++) {
            ...
            choices[i].setOnMouseClicked(e -> {
                writeToServer(Constants.C2S_R4L2_ANS, packageData(finalI));
