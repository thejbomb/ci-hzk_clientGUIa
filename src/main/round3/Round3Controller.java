...
public class Round3Controller extends MainController implements Initializable, Runnable, ServerInteractionInterface, ServerHandlerInterface {
    ...
    protected void show(){
        gp_round3.setVisible(true);
    }

    private void hide(){
        gp_round3.setVisible(false);
    }
    ...
    public void handleServerData(int command, LinkedList<String> data) {
        switch (command) {
            case Constants.BEGIN_R3L1:
                ...
                break;
            case Constants.S2C_R3L1_SCR:
                userData.setRound3Point(Integer.parseInt(data.getFirst()));
                lb_userPoint3.setText(data.getFirst());
                System.out.println(userData.getName() + " current total point for round 3 is " + data.getFirst());
                break;
